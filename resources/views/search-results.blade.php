                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
                    @foreach($products as $product)
                        <a href="{{ route('products.show', $product->id) }}"
                            class="block rounded-xl p-4 hover:shadow-lg transition">
                            ...
                            @else
                                <div class="w-full h-32 bg-gray-100 flex items-center justify-center mb-2">
                                    <span class="material-icons-round text-gray-400 text-4xl">image_not_supported</span>
                                </div>
                            @endif
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    @foreach($blogs as $blog)
                        <a href="{{ route('shop.blog.show', $blog->slug) }}"
                            class="block rounded-xl p-4 hover:shadow-lg transition">
                            @if($blog->image)
                                <img src="{{ $blog->image_url }}" alt="{{ $blog->title }}" class="w-full h-32 object-cover mb-2">
                            @endif
