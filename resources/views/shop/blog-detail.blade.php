    ...
    <div class="bg-white min-h-screen py-0 px-0 font-sans">
        <div class="max-w-6xl mx-auto">
            <div class="pt-12 pb-0 px-0">
                ...
                <h1 class="text-gray-900 font-extrabold text-5xl md:text-7xl leading-tight mb-10 px-2 md:px-0"
                    style="word-break:break-word; letter-spacing:-2px;">{{ $blog->title }}</h1>
            </div>
            <div class="w-full flex flex-col md:flex-row md:gap-8 md:items-start mb-12">
                <div class="md:w-2/3 w-full flex-shrink-0">
                    @if($blog->image)
                        <img src="{{ $blog->image_url }}" alt="{{ $blog->title }}"
                            class="w-full max-h-[480px] object-cover object-center rounded-lg shadow-lg transition-transform duration-300 hover:scale-105"
                            loading="lazy">
                    @else
            ...
            <div class="bg-white px-8 py-12 mt-0 max-w-6xl mx-auto rounded-xl shadow-sm">
                <article class="prose prose-lg max-w-none text-gray-900 leading-relaxed">
                    {!! $blog->content !!}
                    <blockquote class="border-l-4 border-gray-400 pl-4 italic text-gray-700 mt-8">“Chúng tôi luôn nghiên cứu
                        kỹ môi trường sử dụng thực tế để tạo ra sản phẩm phù hợp nhất cho người dùng.”</blockquote>
                </article>
            </div>
        </div>
    </div>
